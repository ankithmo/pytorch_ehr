...
class EHREmbeddings(nn.Module):
    ...
    # cell_type are different for each model variation 
    def __init__(self, input_size, embed_dim, hidden_size, use_cuda, n_layers=1, 
        dropout_r=0.1, cell_type='LSTM', bii=False, time=False, preTrainEmb='', 
        packPadMode = True):
        ...
        if len(input_size) == 1:
            ...
            if len(self.preTrainEmb) > 0:
                ...
                self.embed = nn.Embedding(input_size[0], self.embed_dim, padding_idx=0)#,scale_grad_by_freq=True)
        else:
            if len(input_size) != 3: 
                raise ValueError('the input list is of length 1')
            else: 
                self.multi_emb = True
                self.diag = self.med = self.oth = 1

        #self.emb = self.embed.weight  LR commented Jul 10 19
        if self.time: self.in_size += 1 
        ...
        if self.cell_type == "QRNN": 
            self.bi = 1 # QRNN does not support Bidirectional, DRNN should not be BiDirectional either.
            ...
            self.rnn_c = self.cell(self.in_size, self.hidden_size, 
                                    num_layers = self.n_layers, 
                                    dropout = self.dropout_r, 
                                    bidirectional = bii, batch_first = True)
    ...
    def EmbedPatients_MB(self, mb_t, mtd): #let's define this
        ...
        if self.time:
            ...
            if self.use_cuda: mtd_t.cuda()
            out_emb= torch.cat((embedded, mtd_t), dim=2)
        else:
            out_emb= embedded
        
        if self.use_cuda: out_emb.cuda()
