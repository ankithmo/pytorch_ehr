...
class EHR_RNN(EHREmbeddings):
    def __init__(self, input_size, embed_dim, hidden_size, use_cuda, n_layers=1, 
                    dropout_r=0.1, cell_type='GRU', bii=False , time=False, 
                    preTrainEmb='', packPadMode = True):

       	super(EHR_RNN, self).__init__(input_size, embed_dim, hidden_size, use_cuda,
           n_layers=n_layers, dropout_r=dropout_r, cell_type=cell_type, 
           bii=bii, time=time , preTrainEmb=preTrainEmb, 
           packPadMode=packPadMode)

    #embedding function goes here 
    def EmbedPatient_MB(self, ip, mtd):
        return EHREmbeddings.EmbedPatients_MB(ip, mtd)
    
    def init_hidden(self):
        h_0 = Variable(torch.rand(self.n_layers*self.bi, self.bsize, self.hidden_size))
        if self.use_cuda: h_0.cuda()
        result = (h_0, h_0) if self.cell_type == "LSTM" else h_0
        return result
    
    def forward(self, ip, x_lens, mtd):
        x_in  = self.EmbedPatient_MB(ip, mtd) 
...
class EHR_DRNN(EHREmbeddings): 
    def __init__(self,input_size,embed_dim, hidden_size, use_cuda, n_layers, dropout_r=0.1,cell_type='GRU', bii=False, time=False, preTrainEmb='', packPadMode = False):

       	super(EHR_DRNN, self).__init__(input_size, embed_dim, hidden_size, use_cuda, n_layers=n_layers, dropout_r=dropout_r, cell_type=cell_type,bii=False, time=time , preTrainEmb=preTrainEmb, packPadMode=False)
    ...
    #embedding function goes here 
    def EmbedPatient_MB(self, ip, mtd):
        return self.EmbedPatients_MB(ip, mtd) #EHREmbeddings.EmbedPatients_MB(ip, mtd)
...
#         return EHREmbeddings.EmbedPatients_SMB(self, input,mtd)     
    ...
    def forward(self, ip,  x_lens, mtd, hidden=None):
        x  = self.EmbedPatient_MB(ip, mtd) 
...
class EHR_QRNN(EHREmbeddings):
    def __init__(self,input_size,embed_dim, hidden_size, use_cuda, n_layers=1 ,dropout_r=0.1, cell_type='QRNN', bii=False, time=False, preTrainEmb='', packPadMode = False):

       	super(EHR_QRNN, self).__init__(input_size, embed_dim ,hidden_size, use_cuda, n_layers=n_layers, dropout_r=dropout_r, cell_type=cell_type, bii=bii, time=time , preTrainEmb=preTrainEmb, packPadMode=packPadMode)
    ...
    def EmbedPatient_MB(self, input, mtd):
        return self.EmbedPatients_MB(ip, mtd) #EHREmbeddings.EmbedPatients_MB(input, mtd)
...
class EHR_TLSTM(EHREmbeddings):
    def __init__(self,input_size,embed_dim, hidden_size, use_cuda, n_layers=1, dropout_r=0.1, cell_type='TLSTM', bii=False, time=True, preTrainEmb=''):

        # EHREmbeddings.__init__(self,input_size, embed_dim, hidden_size, use_cuda, n_layers, 
        # dropout_r, cell_type, time , preTrainEmb)
       	super(EHR_TLSTM, self).__init__(input_size, embed_dim, hidden_size, use_cuda, n_layers=n_layers, dropout_r=dropout_r, cell_type=cell_type, bii=False, time=True, preTrainEmb=preTrainEmb, packPadMode=False)
    ...
    def EmbedPatient_SMB(self, ip, mtd):
        return self.EmbedPatients_SMB(ip, mtd) #EHREmbeddings.EmbedPatients_SMB(ip, mtd)
...
class EHR_LR_emb(EHREmbeddings):
    def __init__(self, input_size, embed_dim, use_cuda, time=False, cell_type= 'LR', preTrainEmb=''):
         super(EHR_LR_emb).__init__(input_size, embed_dim, hidden_size = embed_dim, 
                                    use_cuda = use_cuda)
...
class RETAIN(EHREmbeddings):
    def __init__(self, input_size, embed_dim, hidden_size, use_cuda, n_layers):
        super(RETAIN, self).__init__(input_size = input_size, embed_dim=embed_dim,              hidden_size=hidden_size, use_cuda=use_cuda)
