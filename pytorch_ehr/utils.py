...
def trainbatches(mbs_list, model, optimizer, shuffle = True):#,we dont need this print print_every = 10, plot_every = 5): 
    ...
    for i, batch in enumerate(mbs_list):
        ...
        print("seq_l:", len(seq_l))
        print("mtd:", len(mtd))
        output, loss = trainsample(sample, label_tensor, seq_l, mtd, model, 
                                    optimizer, criterion = nn.BCELoss())
        current_loss += loss
        n_iter +=1
...
def epochs_run(epochs, train, valid, test, model, optimizer, shuffle = True, 
                which_model = 'RNN', patience = 20, output_dir = 'models', 
    ...
    for ep in range(epochs):
        start = time.time()
        current_loss, train_loss = trainbatches(mbs_list = train, model= model, 
                                                optimizer = optimizer)
        ...
        train_auc, _, _ = calculate_auc(model = model, mbs_list = train, 
                                        which_model = which_model, 
                                        shuffle = shuffle)
        valid_auc, _, _ = calculate_auc(model = model, mbs_list = valid, 
                                        which_model = which_model, 
                                        shuffle = shuffle)
        valid_time = timeSince(valid_start)
        print(f'\nEpoch ({ep}): Train_auc ({train_auc}), Valid_auc ({valid_auc}), Training Average_loss ({avg_loss}), Train_time ({train_time}), Eval_time ({valid_time})')
        if valid_auc > bestValidAuc: 
              ...
              if test:      
                      ...
                      bestTestAuc, _, _ = calculate_auc(model = best_model, 
                                                        mbs_list = test, 
                                                        which_model = which_model, 
                                                        shuffle = shuffle)
                      print(f'Test_AUC ({bestTestAuc}) , Test_eval_time ({testeval_start})')
    ...
    torch.save(best_model, osp.join(output_dir, model_prefix, model_customed, 
                                    'EHRmodel.pth'))
    torch.save(best_model.state_dict(), osp.join(output_dir, model_prefix, 
                                                    model_customed, 'EHRmodel.st'))
    '''
    ...
    header = 'BestValidAUC|TestAUC|atEpoch'
    logFile = osp.join(output_dir, model_prefix, model_customed, 'EHRmodel.log')
    print2file(header, logFile)
    pFile = f'|{bestValidAuc} |{bestTestAuc} |{bestValidEpoch} '
    print2file(pFile, logFile) 
    if test:
        print(f'BestValidAuc {bestValidAuc} has a TestAuc of {bestTestAuc} at epoch {bestValidEpoch} ')
    else: 
        print(f'BestValidAuc {bestValidAuc} at epoch {bestValidEpoch} ')
        print('No Test Accuracy')
    print(f'Details see ../models/{osp.join(model_prefix, model_customed)}EHRmodel.log')
